package com.kh.practice.chap01;

public class Run {

	public static void main(String[] args) {

		/* 문제 1
		new ControlPractice().practice1(); // ControlPractice()에 정의된 practice1()을 실행한다
		*/
		
		/* 문제 2
		new ControlPractice().practice2(); // ControlPractice()에 정의된 practice2()을 실행한다 
		*/
		
		/* 문제 3
		new ControlPractice().practice3(); // ControlPractice()에 정의된 practice3()을 실행한다
		*/
		
		/* 문제 4
		new ControlPractice().practice4(); // ControlPractice()에 정의된 practice4()을 실행한다 
		*/
		
		/* 문제 5
		new ControlPractice().practice5(); // ControlPractice()에 정의된 practice5()을 실행한다 
		*/
		
		/* 문제 6
		new ControlPractice().practice6(); // ControlPractice()에 정의된 practice6()을 실행한다
		*/
		
		/* 문제 7
		new ControlPractice().practice7(); // ControlPractice()에 정의된 practice7()을 실행한다
		*/
		
		/* 문제 8
		new ControlPractice().practice8(); // ControlPractice()에 정의된 practice8()을 실행한다 
		*/
		
		/* 문제 9 
		new ControlPractice().practice9(); // ControlPractice()에 정의된 practice9()을 실행한다
		*/
		
		/* 문제 10 
		new ControlPractice().practice10(); // ControlPractice()에 정의된 practice10()을 실행한다
		*/
		
		/* 문제 11 (Hard) */
		new ControlPractice().practice11(); // ControlPractice()에 정의된 practice11()을 실행한다
		
	}

}
