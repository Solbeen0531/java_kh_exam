package com.kh.practice.repeat;

import java.util.Scanner;

public class RepeatPractice12 {

	public static void main(String[] args) {
		
		/* 문제 12
		정수 두 개와 연산자를 입력 받고 입력된 연산자에 따라 알맞은 결과를 출력하세요.
		단, 해당 프로그램은 연산자 입력에 “exit”라는 값이 들어올 때까지 무한 반복하며
		exit가 들어오면 “프로그램을 종료합니다.”를 출력하고 종료합니다.
		또한 연산자가 나누기이면서 두 번째 정수가 0으로 들어오면
		“0으로 나눌 수 없습니다. 다시 입력해주세요.”를 출력하며,
		없는 연산자가 들어올 시 “없는 연산자입니다. 다시 입력해주세요.”라고 출력하고
		두 경우 모두 처음으로 돌아가 사용자가 다시 연산자부터 입력하도록 하세요.
		
		연산자(+, -, *, /, %) : +
		정수1 : 10
		정수2 : 4
		10 + 4 = 14

		연산자(+, -, *, /, %) : / 		연산자(+, -, *, /, %) : /
		정수1 : 10 						정수1 : 10
		정수2 : 4 						정수2 : 0
		10 / 4 = 2 						0으로 나눌 수 없습니다. 다시 입력해주세요.
		
		연산자(+, -, *, /, %) : ^ 		연산자(+, -, *, /, %) : exit
		정수1 : 10 						프로그램을 종료합니다.
		정수2 : 4
		없는 연산자입니다. 다시 입력해주세요.
		*/

		Scanner sc = new Scanner(System.in); // 스캐너 시작

		String calc = "apple"; // 반복문을 수행하기 위해 exit가 아닌 초기값 부여

		while (!(calc.equals("exit"))) { // calc 값이 exit가 아니라면 무한 반복

			System.out.print("연산자(+, -, *, /, %) : ");
			calc = sc.nextLine();

			if (calc.equals("exit")) { // calc 값이 exit라면 프로그램 종료
				System.out.println("프로그램을 종료합니다.");
				break;
			}

			System.out.print("정수1 : ");
			int numA = sc.nextInt(); // 정수형 숫자 입력

			System.out.print("정수2 : ");
			int numB = sc.nextInt(); // 정수형 숫자 입력
			sc.nextLine();

			switch (calc) {
			case "+":
				System.out.println(numA + " + " + numB + " = " + (numA + numB));
				break;
			case "-":
				System.out.println(numA + " - " + numB + " = " + (numA - numB));
				break;
			case "*":
				System.out.println(numA + " * " + numB + " = " + (numA * numB));
				break;
			case "/":
				if ((numA == 0) || (numB == 0)) { // 정수 둘 중 하나가 0이라면 출력
					System.out.println("0으로 나눌 수 없습니다. 다시 입력해주세요.");
				} else {
					System.out.println(numA + " / " + numB + " = " + (numA / numB));
				}
				break;
			case "%":
				System.out.println(numA + " % " + numB + " = " + (numA % numB));
				break;
			default:
				System.out.println("없는 연산자입니다. 다시 입력해주세요.");
				break;
			} // switch (calc) {
			
			System.out.println();

		} // while (!(calc.equals("exit"))) {

		sc.close(); // 스캐너 끝
	}

}
